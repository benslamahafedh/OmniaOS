...
const Desktop: React.FC = () => {
  ...
  const quickActions: QuickAction[] = [
    ...
    },
    {
      ...
      action: () => window.open('https://www.omniasamantha.fun/', '_blank'),
      category: 'AI'
    },
    {
      id: 'add-widget',
            ...
            {!isMobile && (
              <div className="flex items-center space-x-3">
                ...
                <a 
                  ...
                  className="p-2 rounded-lg bg-pink-800/10 hover:bg-pink-800/20 cursor-pointer transition-all duration-300 group border border-pink-700/20"
                >
                  <svg width="16" height="16" fill="currentColor" className="text-gray-200 group-hover:text-white" viewBox="0 0 24 24">
        ...
        `}>
          ...
          `}>
            {folders.map((folder, i) => (
              <div
                key={folder.id}
                onClick={() => folder.id === 'samantha' ? window.open('https://www.omniasamantha.fun/', '_blank') : handleWindowOpen(`folder-${folder.id}`)}
                className={`
              ...
              `}>
                ...
                `}>
                  ...
                  {[
                    ...
                    { icon: <Sparkles size={isMobile ? 24 : 20} />, action: () => window.open('https://www.omniasamantha.fun/', '_blank'), label: 'Samantha' },
