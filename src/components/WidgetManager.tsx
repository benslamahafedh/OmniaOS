...
const WidgetManager: React.FC<WidgetManagerProps> = ({
  ...
  const renderWidget = (widget: Widget) => {
    switch (widget.type) {
      ...
      default:
        return <div className="p-4 text-orange-400">Unknown widget type</div>;
    }
  };
    ...
    <>
      ...
      {showWidgetPicker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-gray-900/95 backdrop-blur-xl rounded-2xl border border-orange-500/30 p-6 w-full max-w-2xl mx-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {widgetTypes.map(widgetType => (
                <button
                  ...
                  className="p-4 rounded-xl border border-orange-500/20 hover:border-orange-500/40 bg-orange-500/5 hover:bg-orange-500/10 transition-all text-left"
                >
                  <div className="flex items-center gap-3 mb-2">
                    <div className="text-orange-400">{widgetType.icon}</div>
      ...
      <button
        ...
        className="fixed top-20 right-8 z-20 bg-orange-500/20 hover:bg-orange-500/30 text-orange-400 p-3 rounded-xl border border-orange-500/30 transition-all duration-300 hover:scale-105"
        title="Add Widget"
      >
        <Plus size={20} />
      </button>
      ...
      {widgets.map(widget => (
        <div
          key={widget.id}
          className="fixed z-30 bg-black/90 backdrop-blur-xl border border-orange-500/30 rounded-2xl overflow-hidden shadow-xl"
          style={{
            left: widget.x,
            top: widget.y,
            width: widget.width,
        ...
        >
          {/* Widget Header */}
                      <div className="flex items-center justify-between p-3 bg-orange-500/10 border-b border-orange-500/20 cursor-move">
      ...
      {[
        ...
        <div key={metric.label} className="space-y-2">
          ...
          <div className="h-2 bg-orange-500/20 rounded-full">
            <div 
              className="h-2 bg-gradient-to-r from-orange-500 to-rose-600 rounded-full transition-all duration-500"
...
const ClockWidget: React.FC = () => {
  ...
  return (
    <div className="p-4 text-center h-full flex flex-col justify-center">
      <div className="text-3xl font-bold text-orange-400 tabular-nums mb-2">
  ...
  return (
    <div className="p-4 h-full">
      <div className="text-center mb-4">
        <div className="text-orange-400 font-semibold">{months[currentMonth]} {currentYear}</div>
      </div>
      <div className="grid grid-cols-7 gap-1 text-xs">
        {days.map(day => (
          <div key={day} className="text-center text-orange-400/60 font-medium p-1">
            {day[0]}
          </div>
        ))}
  ...
  return (
    <div className="p-4 h-full">
      <div className="flex justify-between items-center mb-3">
        <div className="text-orange-400 font-medium text-sm">Performance History</div>
        <div className="flex gap-3 text-xs">
          <div className="flex items-center gap-1">
            <div className="w-2 h-2 bg-orange-500 rounded"></div>
            <span className="text-orange-400/80">CPU</span>
          </div>
          <div className="flex items-center gap-1">
            <div className="w-2 h-2 bg-orange-500 rounded"></div>
      ...
      <div className="h-32 bg-orange-500/5 rounded-lg relative overflow-hidden">
...
const QuickStatsWidget: React.FC<{ systemMetrics: any }> = ({ systemMetrics }) => (
  <div className="p-4 h-full">
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="text-orange-400/80 text-sm">System Status</div>
        <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
      </div>
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div>
          <div className="text-orange-400/60">CPU</div>
          <div className="text-white font-medium">{systemMetrics.cpu}%</div>
        </div>
        <div>
          <div className="text-orange-400/60">RAM</div>
          <div className="text-white font-medium">{systemMetrics.memory}%</div>
        </div>
        <div>
          <div className="text-orange-400/60">Temp</div>
          <div className="text-white font-medium">{systemMetrics.temperature}°C</div>
        </div>
        <div>
          <div className="text-orange-400/60">Uptime</div>
